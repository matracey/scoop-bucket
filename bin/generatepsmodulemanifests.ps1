...
while ($ManifestQueue.Count -gt 0) {
    ...
    $TemplateObj = [ordered]@{
        ...
        autoupdate    = [ordered]@{ url = "https://psg-prod-eastus.azureedge.net/packages/$LowerName.`$version.nupkg" }
        hash          = 'TODO'
        url           = "https://psg-prod-eastus.azureedge.net/packages/$LowerName.$($Module.Self.Version.ToString()).nupkg"
